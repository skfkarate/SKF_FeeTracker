...
function requestTooLarge(request: Request) {
  ...
  return Number.isFinite(parsedLength) && parsedLength > MAX_GALLERY_PHOTO_BODY_BYTES;
}
  ...
  try {
    if (requestTooLarge(request)) {
      return Response.json(
        {
          success: false,
          error: `Request body exceeds ${MAX_GALLERY_PHOTO_BODY_BYTES} bytes.`,
    ...
    const response = await fetch(
      karateBackendUrl("/api/integrations/feetrack/gallery/upload"),
      {
        method: "POST",
        headers: {
    ...
    return Response.json(data || { success: false, error: "Gallery photo upload failed." }, {
    ...
    const message =
      name === "AbortError" || name === "TimeoutError"
        ? "Gallery photo upload timed out. Please retry."
